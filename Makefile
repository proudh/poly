...
NV := $$($(GLIDE) novendor)
BASEPKG := github.com/dvonthenen/poly
...
EMCCODE := $(GOPATH)/src/github.com/dvonthenen
...
build_:
	@echo "target: build"
	@printf "  ...building poly $(V_OS_ARCH)..."; \
		cd $(BASEDIR); \
		FILE=$(BINDIR)/$(V_OS_ARCH)/poly; \
		env GOOS=$(_GOOS) GOARCH=$(_GOARCH) go clean -i $(VERSIONPKG); \
		env GOOS=$(_GOOS) GOARCH=$(_GOARCH) go build -o $$FILE $(GOFLAGS) $(LDFLAGS) ./poly; \
		$(PRINT_STATUS); \
		if [ "$$EC" -eq "0" ]; then \
			mkdir -p $(DEPLOY)/$(V_OS_ARCH); \
			mkdir -p $(DEPLOY)/latest; \
			cd $(BINDIR)/$(V_OS_ARCH); \
			TARBALL=poly-$(V_OS_ARCH)-$(V_SEMVER).tar.gz; \
			LATEST=poly-$(V_OS_ARCH).tar.gz; \
			tar -czf $$TARBALL poly; \
...
_install: _deps _fmt
	@echo "target: install"
	@printf "  ...installing poly $(V_OS_ARCH)..."; \
		cd $(BASEDIR); \
		go clean -i $(VERSIONPKG); \
		go install $(GOFLAGS) $(LDFLAGS) ./poly/; \
		$(PRINT_STATUS); \
		if [ "$$EC" -eq "0" ]; then \
			FILE=$(GOPATH)/bin/poly; \
			BYTES=$$($(STAT_FILE_SIZE)); \
			SIZE=$$(($$BYTES / 1024 / 1024)); \
...
rpm:
	...
	@printf "  ...building rpm $(V_ARCH)..."; \
		...
		if [ "$$EC" -eq "0" ]; then \
			...
			FILE=$$DEPLOY_FILE; \
			cp -f $$FILE $(DEPLOY)/latest/poly-latest-$(V_ARCH).rpm; \
			BYTES=$$($(STAT_FILE_SIZE)); \
			SIZE=$$(($$BYTES / 1024 / 1024)); \
...
deb:
	...
	@printf "  ...building deb $(V_ARCH)..."; \
		...
		if [ "$$EC" -eq "0" ]; then \
			...
			DEPLOY_FILE=$(DEPLOY)/$(V_OS_ARCH)/$$(basename $$FILE); \
			FILE=$$DEPLOY_FILE; \
			cp -f $$FILE $(DEPLOY)/latest/poly-latest-$(V_ARCH).deb; \
			BYTES=$$($(STAT_FILE_SIZE)); \
			SIZE=$$(($$BYTES / 1024 / 1024)); \
